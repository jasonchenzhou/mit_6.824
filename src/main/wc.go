...
func Map(value string) *list.List {
	//fmt.Println(value)
	arr := strings.FieldsFunc(value, Divide)
	l := list.New()
	for _, wd := range arr{
		l.PushBack(mapreduce.KeyValue{wd, "1"})
	}
	//fmt.Println(l.Len())
	return l
}
...
func Reduce(key string, values *list.List) string {
	...
	for pair := values.Front(); pair != nil; pair = pair.Next(){
		tmp, _ := strconv.Atoi(pair.Value.(string))
	    sum += tmp
	}
